...
const ACTIONS = [
  ...
  { label: 'Type',     icon: Send,  type: 'type' },
...
export default function MirrorMode() {
  ...
  const broadcast = async () => {
    ...
    // Real command broadcast — update each session with the command in userMetadata
    ...
    setBroadcastLog(prev => [
      {
        ...
        success: successCount === selectedIds.size,
        successCount,
      },
      ...prev.slice(0, 19),
    ]);

    toast.success(`Command broadcast to ${successCount} sessions`);
    ...
    <div className="p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Broadcast commands to all selected running sessions</p>
        </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex gap-1">
              <Button size="sm" variant="ghost" onClick={() => setSelectedIds(new Set(sessions.map(s => s.id)))}
          ...
          {sessions.length === 0 ? (
            <div className="text-center py-8 text-gray-600 text-xs">Load running sessions to select targets</div>
          ) : (
            <div className="space-y-1.5 max-h-72 overflow-y-auto">
              {sessions.map(s => (
                <div key={s.id} onClick={() => toggleSession(s.id)}
                  ...
                  <span className="text-xs font-mono text-gray-300 truncate flex-1">{s.id.slice(0, 16)}…</span>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-4">
          ...
          {actionType === 'navigate' && (
            <div>
              ...
              <Input placeholder="https://example.com" value={masterUrl}
          ...
          {(actionType === 'click' || actionType === 'type') && (
            <div>
              ...
              <Input placeholder="#submit-btn or .form-input" value={selector}
          ...
          {actionType === 'type' && (
            <div>
              ...
              <Input placeholder="Hello from Mirror Mode" value={textInput}
                onChange={e => setTextInput(e.target.value)}
                className="bg-gray-800 border-gray-700 text-gray-200 text-sm" />
            </div>
          )}
          {actionType === 'screenshot' && (
            <p className="text-xs text-gray-500">Screenshots will be captured via session metadata update.</p>
          )}

          <Button onClick={broadcast} disabled={broadcasting || selectedIds.size === 0}
            ...
            <div className="space-y-2 max-h-80 overflow-y-auto">
              {broadcastLog.map(entry => (
                <div key={entry.id} className="bg-gray-800/50 rounded-lg px-3 py-2 space-y-1">
                  <div className="flex items-center justify-between">
                    <Badge className="bg-emerald-500/10 text-emerald-400 border-emerald-500/30 text-xs">{entry.action}</Badge>
                    <span className="text-xs text-gray-600">{entry.timestamp}</span>
                  </div>
                  <div className="text-xs text-gray-400 truncate">{entry.payload || '—'}</div>
                  <div className="text-xs text-gray-600">→ {entry.successCount}/{entry.targets} succeeded</div>
                </div>
              ))}
            </div>
          )}
