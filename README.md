...
Optional — for local development only. You can run against the published Base44 backend without an interactive Google login by setting a Base44 API key. When set, the SDK authenticates every request via the `api_key` header and skips the user-session flow. (This mirrors the documented server-side SDK pattern.)
